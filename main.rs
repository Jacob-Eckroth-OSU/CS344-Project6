...
fn main() {
	...
	let left_half = xs[0].to_vec();
	let right_half = xs[1].to_vec();

	
		
	
	let t1 = thread::spawn(move || map_data(&left_half));
	let t2 = thread::spawn(move || map_data(&right_half));
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
	...
	for i in 0.. v.len(){
		return_vec[i%num_partitions].push(v[i]);
	}
